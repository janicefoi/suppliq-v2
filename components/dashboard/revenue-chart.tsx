  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white shadow-sm px-3 py-2 text-xs">
      ...
      <p className="font-bold text-slate-900 tabular-nums">
        {new Intl.NumberFormat(undefined, { style: "currency", currency, minimumFractionDigits: 2 }).format(payload[0].value)}
      </p>
    </div>
  );
}
...
export function RevenueChart({ data, currency }: { data: DailyRevenue[]; currency: string }) {
  const today = new Date().toLocaleDateString(undefined, { weekday: "short", day: "2-digit" })
    .replace(",", "").trim();
