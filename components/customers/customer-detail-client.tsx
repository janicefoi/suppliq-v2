  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total spent", value: formatCurrency(totalSpent, currency), sub: "all time", icon: TrendingUp, color: "text-blue-600" },
          { label: "Purchases", value: String(customer.sales.length), sub: "all time", icon: ShoppingBag, color: "text-slate-700" },
          { label: "Total paid (credit)", value: formatCurrency(customer.creditPayments.reduce((s, p) => s + Number(p.amount), 0), currency), sub: "all time", icon: Receipt, color: "text-green-600" },
          { label: "Outstanding", value: formatCurrency(balance, currency), sub: "current balance", icon: CreditCard, color: hasCredit ? "text-red-600" : "text-green-600" },
        ...
        <div className="flex items-center gap-3">
          ...
          {period !== "all" && (
            <span className="text-xs text-slate-500">
              {filteredSales.length} sale{filteredSales.length !== 1 ? "s" : ""} · {formatCurrency(periodSpent, currency)} spent
              {periodPaid > 0 && ` · ${formatCurrency(periodPaid, currency)} paid`}
            </span>
          )}
        </div>
      )}
                ...
                <>
                  {filteredSales.map((sale) => (
                    <TableRow key={sale.id} className="hover:bg-slate-50/60">
                      ...
                      <TableCell className="text-right text-xs font-semibold tabular-nums text-slate-800">{formatCurrency(sale.totalAmount, currency)}</TableCell>
                      <TableCell>
                  ...
                  {filteredSales.length > 1 && (
                    <TableRow className="bg-slate-50 font-semibold">
                      ...
                      <TableCell className="text-right text-xs tabular-nums text-slate-900">{formatCurrency(periodSpent, currency)}</TableCell>
                      <TableCell />
                    </TableRow>
                  )}
                </>
      ...
      <div className="customer-statement-print" style={{ display: "none" }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, marginBottom: 16 }}>
          <thead>
            <tr style={{ borderBottom: "1px solid #0f172a" }}>
              ...
              <th style={{ textAlign: "right", padding: "4px 6px" }}>Total ({currency})</th>
          ...
          <tbody>
            {filteredSales.length === 0 ? (
              ...
              filteredSales.map((s) => (
                <tr key={s.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                  ...
                  <td style={{ padding: "4px 6px", textAlign: "right" }}>{Number(s.totalAmount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
            ...
            {filteredSales.length > 0 && (
              <tr style={{ borderTop: "1px solid #0f172a", fontWeight: 700 }}>
                ...
                <td style={{ padding: "4px 6px", textAlign: "right" }}>{periodSpent.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td />
              </tr>
            )}
          </tbody>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, marginBottom: 16 }}>
          <thead>
            <tr style={{ borderBottom: "1px solid #0f172a" }}>
              <th style={{ textAlign: "left", padding: "4px 6px" }}>Date</th>
              <th style={{ textAlign: "right", padding: "4px 6px" }}>Amount ({currency})</th>
